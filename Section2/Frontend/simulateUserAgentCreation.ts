...
function uint8ArrayToFixedLengthText(
  uint8Array: Uint8Array,
  length: number
): string {
  const base64 = btoa(String.fromCharCode.apply(null, Array.from(uint8Array)));
  const alphanumericOnly = base64.replace(/[^A-Za-z0-9]/g, "");
  const truncatedOrPadded = alphanumericOnly
    .slice(0, length)
    .padEnd(length, "0");
...
const getWalletKey = async () => {
  ...
  try {
    ...
    store = await Store.open({
      ...
      profile: "regov",
      passKey: process.env.PASSWORD_INSERTED_BY_USER,
    });
  } catch {
...
const createAgent = async () => {
  ...
  const config: InitConfig = {
    label: "user-agent",
    walletConfig: {
      id: process.env.WALLET_ID as string,
      key: walletKey,
    },
  };
